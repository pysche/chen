	...
	if (json.statusCode == DWZ.statusCode.ok){
		...
		switch (json.callbackType) {
			case 'closeCurrent':
				...
				break;
			case 'forproductnames':
				try {
					$('#datalist_productnames_left tr.selected').eq(0).trigger('click');
				} catch (e) {
				...
				$.pdialog.closeCurrent();
				break;
			case 'forproductrepair':
				try {
					$('#datalist_productrepair_left tr.selected').eq(0).trigger('click');
				} catch (e) {
					console.log(e.message);
				}
