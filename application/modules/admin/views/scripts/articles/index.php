<div class="pageHeader">
	...
	<div class="searchBar">
		<ul class="searchContent">
			<li>
			<label>所在分类:</label>
			<?php echo $this->FormSelect('Category', $_REQUEST['Category'], null, Crm_Funcs::array_merge(array(''=>'* 请选择  *'), $this->categories));?>
			</li>
			<li>
				<label>文章标题:</label>
...
<div class="pageContent">
	...
	<table class="table" width="100%" layoutH="138">
		<thead>
		<tr>
			<th width="45%" orderField="Title" <?php if ($_REQUEST['orderField'] == 'Title') { echo 'class="'.$_REQUEST['orderDirection'].'"'; }?>>标题</th>
			<th width='15%' orderField='Category' <?php if ($_REQUEST['orderField'] == 'Category') { echo 'class="'.$_REQUEST['orderDirection'].'"';}?>>分类</th>
			<th width='10%' orderField="SortNum" <?php if ($_REQUEST['orderField'] == 'SortNum') { echo 'class="'.$_REQUEST['orderDirection'].'"'; }?>>排序</th>
			<th width='10%' orderField='PubFlag' <?php if ($_REQUEST['orderField'] == 'PubFlag') { echo 'class="'.$_REQUEST['orderDirection'].'"';}?>>是否发布</th>
		...
		<?php foreach($this->list as $vo){?>
			<tr target="sid_obj" rel="<?php echo $vo->id;?>">
				...
				<td><?php echo $this->categories[$vo->Category];?></td>
