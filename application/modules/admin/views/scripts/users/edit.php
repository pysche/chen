...
<div class="pageContent">
	<div class="pageFormContent" layoutH="58">
		<dl>
			<dt>用户名：</dt><dd><input name="Username" class="required" alt="英文字母加数字" type="text" value='<?php echo $this->vo->Username;?>' /></dd>
		</dl>
		<dl>
			<dt>密码：</dt><dd><input name="Password" type="password"/></dd>
		</dl>
		<dl>
			<dt>确认密码：</dt><dd><input name="Password2" type="password"/></dd>
		</dl>
		<dl>
			<dt>姓名：</dt><dd><input name="Realname" class="required" type="text" value="<?php echo $this->vo->Realname;?>" /></dd>
		</dl>
		<dl class='block'>
			<dt>权限：</dt>
			<dd class='auth_pane'>
					...
					<li>
						<dl>
							...
							<dd>
								...
								<?php 
									if (isset($this->vo->AuthMask[$as['key']]) && in_array('list', $this->vo->AuthMask[$as['key']])) {
								?>
								checked='checked'
								<?php
									}
								...
								<?php 
									if (isset($this->vo->AuthMask[$as['key']]) && in_array('add', $this->vo->AuthMask[$as['key']])) {
								?>
								checked='checked'
								<?php
									}
								...
								<?php 
									if (isset($this->vo->AuthMask[$as['key']]) && in_array('update', $this->vo->AuthMask[$as['key']])) {
								?>
								checked='checked'
								<?php
									}
								...
								<?php 
									if (isset($this->vo->AuthMask[$as['key']]) && in_array('delete', $this->vo->AuthMask[$as['key']])) {
								?>
								checked='checked'
								<?php
									}
								...
								<?php 
									if (isset($this->vo->AuthMask[$as['key']]) && in_array('audit', $this->vo->AuthMask[$as['key']])) {
								?>
								checked='checked'
								<?php
									}
