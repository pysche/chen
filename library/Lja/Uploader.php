...
class Lja_Uploader
{
	...
	{
		Lja_Log::getInstance()->debug(var_export($params, true));
		$file = &$params['file'];
		$hash = &$params['hash'];
		...
		$config = &Lja_Config::appConfig()->toArray();
		...
		Lja_Db::t('attachment')->insert($s);
		$storage = &Lja_File_Storage::factory($config['attachment']['save']['protocol']);
	...
	public static function Del($fid)
	{
		...
		Lja_Db::t('attachment')->doDelete($fid);
	}
}